...
| `FLOCI_GCP_SERVICES_IAM_AUTHORIZATION_MODE` | `disabled` | `enforce` enables Resource Manager project IAM for Floci-issued service-account tokens. Default `disabled` keeps permissive behavior. [Coverage and limits](../services/iam.md#opt-in-enforcement) |
