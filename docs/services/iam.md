...
| `FLOCI_GCP_SERVICES_IAM_AUTHORIZATION_MODE` | `disabled` | IAM allow-policy evaluation mode. `disabled` preserves no-auth behavior; `enforce` applies stored policies to supported bucket operations and filters bucket `testIamPermissions` responses |
...
by the stored bucket policy. Supported GCS REST bucket metadata, bucket
IAM-policy, retention-lock, storage-layout, and notification operations are
also checked against their documented bucket permissions. GCS gRPC, object,
and ACL operations are not yet restricted by IAM allow policies.
