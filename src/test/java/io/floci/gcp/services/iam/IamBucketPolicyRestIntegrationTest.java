...
class IamBucketPolicyRestIntegrationTest {
    ...
    @Test
    void preservesUblaForPartialIamConfigurationPatchesWithConditionalPolicy() {
        String bucket = createBucket(true);
        setPolicy(bucket, conditionalPolicy("resource.name.startsWith('projects/_/buckets/" + bucket + "/objects/reports/')"))
                .statusCode(200);
        ...
        given().contentType("application/json")
                .body(Map.of("iamConfiguration", Map.of("publicAccessPrevention", "enforced")))
                .when().patch("/storage/v1/b/" + bucket)
                .then().statusCode(200)
                .body("iamConfiguration.uniformBucketLevelAccess.enabled", equalTo(true));
    }
    ...
    @Test
    void rejectsRemovingIamConfigurationWhileConditionalPolicyRemainsConfigured() {
        String bucket = createBucket(true);
        setPolicy(bucket, conditionalPolicy("resource.name.startsWith('projects/_/buckets/" + bucket + "/objects/reports/')"))
                .statusCode(200);
    ...
    @Test
    void canonicalTestPermissionsRequiresBucketScopedPermissions() {
        String bucket = createBucket(false);

        given().when().get("/storage/v1/b/" + bucket + "/iam/testPermissions")
                .then().statusCode(400).body("error.status", equalTo("INVALID_ARGUMENT"));

        given().queryParam("permissions", "storage.buckets.create")
                .when().get("/storage/v1/b/" + bucket + "/iam/testPermissions")
                .then().statusCode(400).body("error.status", equalTo("INVALID_ARGUMENT"));

        given().queryParam("permissions", "storage.buckets.list")
                .when().get("/storage/v1/b/" + bucket + "/iam/testPermissions")
                .then().statusCode(400).body("error.status", equalTo("INVALID_ARGUMENT"));

        given().queryParam("permissions", "storage.objects.get", "storage.objects.get")
                .when().get("/storage/v1/b/" + bucket + "/iam/testPermissions")
                .then().statusCode(200)
                .body("permissions", equalTo(List.of("storage.objects.get", "storage.objects.get")));
    }
