...
|---|---|
| Resource Manager v1 project metadata and project policies | REST JSON; project bindings also inherit to supported children |
| Pub/Sub v1 topics, subscriptions, snapshots | Implemented REST JSON and gRPC operations, including the IAM mixin and StreamingPull |
| Secret Manager v1 secrets and versions | Implemented REST JSON and gRPC operations |
...
Supported resource policies and project policies are evaluated together. Sibling
policies never inherit. Every permission for a multi-resource operation must pass:
for example, creating a subscription needs create permission on its project and
attach permission on its source topic, including across projects.
...
- `roles/pubsub.publisher`, `subscriber`, `viewer`, `editor`, and `admin`.
- `roles/secretmanager.secretAccessor`, `secretVersionAdder`,
  `secretVersionManager`, `viewer`, and `admin`.
- `roles/browser` and `roles/resourcemanager.projectIamAdmin`.
- `roles/owner`, `roles/editor`, and `roles/viewer`, restricted to the supported
  services' explicitly declared permissions. Editor/viewer do not grant secret
  payload access or Pub/Sub IAM policy reads.
...
callers in enforce mode; use each excluded service's own documented policy API. Unsupported CEL expressions produce
...
and request-time comparison. The CEL standard library is not enabled. Secret
versions evaluate their version resource name while reading their parent secret's
policy. The catalog and condition profile are subsets of Google Cloud IAM.
...
- Resource names are evaluated as requested. Project ID/number aliases and secret
  version aliases are not canonicalized to equivalent numeric resource names.
- Only declared operation permissions are checked. Dependent service-agent,
  ...
  in GCP.
- StreamingPull checks each incoming message using the stream credential and
  current subscription. A policy change does not proactively terminate an idle
  stream or reauthorize asynchronous delivery callbacks.
