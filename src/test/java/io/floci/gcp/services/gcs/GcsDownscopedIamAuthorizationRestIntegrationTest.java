...
class GcsDownscopedIamAuthorizationRestIntegrationTest {
    ...
    void downscopedImpersonatedTokenRequiresBothCabAndBucketPolicy() {
        ...
        given().header("Authorization", bearer(authorizedToken))
                .contentType("text/csv").body("replacement")
                .queryParam("uploadType", "media").queryParam("name", "allowed/report.csv")
    ...
    private String cabOptions() {
        ...
        return """
                {"accessBoundary":{"accessBoundaryRules":[{
                  ...
                  "availablePermissions":["inRole:roles/storage.legacyObjectReader","inRole:roles/storage.objectViewer","inRole:roles/storage.legacyBucketWriter"],
